...
fn walk_directory(path: &Path) -> Result<ResultCount, CLIError> {
    ...
    if !path.is_dir() {
        return Err(CLIError::new(format!("Expected {} to be a directory, but it was not", path.display()), false))
    }
    for entry in path.read_dir().expect("Failed to read directory contents") {
...
fn run_test_file(path: &Path) -> Result<ResultCount, CLIError> {
    ...
    let file_contents = match fs::read_to_string(path) {
        Ok(res) => res,
        Err(_) => return Err(CLIError::new(format!("Failed to read file {}", path.display()), false))
    };
    match ChimeraScriptAST::new(file_contents.as_str()) {
        Ok(ast) => {
            ...
            let results = if test_name.is_some() {
                ...
                frontend::run_function_by_name(ast, path.file_name().expect("Failed to get file name when running functions"), test_name.as_str())
            }
            else {
                frontend::run_functions(ast, path.file_name().expect("Failed to get file name when running functions"))
            };
...
fn main() {
    ...
    TEST_NAME.set(args.name).expect("Failed to set TEST_NAME OnceLock");

    // Get path from args
    let path = PathBuf::from_str(args.path.as_str()).expect("infallible method failed when creating PathBuf from str");
    ...
    let results = if path_slice.is_dir() {
        ...
        Err(CLIError::new("The given path was not a directory or file, when it must be one of those two".to_string(), false))
    };
...
mod main_tests {
    use std::path::Path;
    use crate::{run_test_file, TEST_NAME, walk_directory};
    ...
    fn test_run_test_file() {
        ...
        assert!(res.is_err(), "Trying to run a file that does not exist should error");
        ...
        assert!(res.is_err(), "Trying to run a file that does not have a .chs extension should error");
        ...
        let res = run_test_file(wrong_type_path);
        assert!(res.is_err(), "Trying to run a test file but passing a directory should error");
